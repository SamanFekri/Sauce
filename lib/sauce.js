...
function Sauce(sauces) {
  console.log(`Sauce.js get this sauces:`);
  console.log(sauces);

  for (let k = 0; k < sauces.length; k++) {
    let temp = sauces[k];
    temp.size = document.getElementsByTagName(temp.name).length;
    console.log(`Start getting <${temp.name}>`);
    $.ajax(
      {
        url: temp.src,
        dataType: 'sue',
        error: function (xhr, status, err) {
          console.error(`This error occurred while compiling the <${temp.name}>`);
          console.error(err);
        },
        success: function (data) {
          console.log(`Get <${temp.name}> was successful.`);
          // console.log(data);
          temp.id = data.id.trim();
          temp.style = data.style.trim();
          temp.template = data.template.trim();
          temp.data = data.data.trim();
          temp.other = data.other.trim();
          temp.size = 0;

          let script = $("<script />", {html: 'let ' + temp.id + ' = [];'});
          $("body").append(script);

          renderSameItems(temp);

          let parser = new cssjs();
          //parse css string
          let parsed = parser.parseCSS(temp.style);
          for (let j = 0; j < parsed.length; j++) {
            parsed[j].selector = temp.name + ' ' + parsed[j].selector;
          }

          let style = $("<style />", {html: parser.getCSSForEditor(parsed)});
          $("body").append(style);
        },
        accepts: {
          sue: 'application/x-some-custom-type'
        },
        // Instructions for how to deserialize a `mycustomtype`
        converters: {
          'text sue': function(result) {
            let newresult = {};

            newresult['id'] = result.match(/<id.*>((.|\n)*?)<\/id>/)[1];
            newresult['template'] = result.match(/<template.*>((.|\n)*?)<\/template>/)[1];
            newresult['data'] = result.match(/<data.*>((.|\n)*?)<\/data>/)[1];
            newresult['other'] = result.match(/<other.*>((.|\n)*?)<\/other>/)[1];
            newresult['style'] = result.match(/<style.*>((.|\n)*?)<\/style>/)[1];
  ...
  this.renderAll = function () {
    for (let i = 0; i < sauces.length; i++) {
      renderSameItems(sauces[i])
    }
  };

  this.render = function (item) {
    for (let i = 0; i < sauces.length; i++) {
      if (item.tagName.toLowerCase() === sauces[i].name) {
        let tmp = $.extend(true, {}, sauces[i]);
        renderItem(item, tmp, tmp.size, sauces[i]);
        return;
      }
    }
    console.log(`<${item.tagName}> is not exist in sauces`);
  };

  this.addSauce = function (sauce) {
    for (let i = 0; i < sauces.length; i++) {
      if (sauce.name === sauces[i].name) {
        console.log(`<${sauces[i].name}> already exists`);
        return;
      }
    }
...
function isDataTag(sauces, name) {
  if (!name.toLowerCase().startsWith('data:'))
    return false;
  for (let i = 0; i < sauces.length; i++) {
    if (sauces[i].name.toLowerCase() === name.toLowerCase()) {
      return false;
    }
  }
  return true;
...
function renderSameItems(temp) {
  let items = document.getElementsByTagName(temp.name);
  // console.log(items);

  for (let i = 0; i < items.length; i++) {
    let tmp = $.extend(true, {}, temp);

    let needRender = true;
    if ($(items[i]).attr('status')) {
      if ($(items[i]).attr('status').toLowerCase() === 'rendered') {
        needRender = false;
      }
    }

    if (needRender) {
      renderItem(items[i], tmp, i, temp)
    } else {
      console.log(`Do no need render => <${tmp.name}> id: ${tmp.id}[${i}]`);
    }
  }
}

function renderItem(parent, tmp, i, realSauce) {

  console.log(`Start rendering <${tmp.name}> id: ${tmp.id}[${i}]`);

  // let parent = items[i];

  // get all elements
  let elements = $.extend(true, {}, parent.children);
  for (let j = 0; j < elements.length; j++) {
    if (isDataTag(sauces, elements[j].tagName)) {
      if (tmp.data == '') {
        tmp.data = 'data: {}';
        let n = tmp.data.lastIndexOf("}");
        tmp.data = tmp.data.slice(0, n) + tmp.data.slice(n).replace("}",
          elements[j].tagName.split(':')[1].toLowerCase().replace(/-/g, '_') + ": `" + elements[j].innerHTML.trim().replace(/`/g, '\\`') + "`}");
      } else {
        let n = tmp.data.lastIndexOf("}");
        tmp.data = tmp.data.slice(0, n) + tmp.data.slice(n).replace("}",
          "," + elements[j].tagName.split(':')[1].toLowerCase().replace(/-/g, '_') + ": `" + elements[j].innerHTML.trim().replace(/`/g, '\\`') + "`}");
  ...
  for (let j = 0; j < attrs.length; j++) {
    if (attrs[j].name !== 'id') {
      if (attrs[j].name.toLowerCase().startsWith('data:')) {
        if (tmp.data == '') {
          tmp.data = 'data: {}';
          let n = tmp.data.lastIndexOf("}");
          tmp.data = tmp.data.slice(0, n) + tmp.data.slice(n).replace("}",
            attrs[j].name.replace(/-/g, '_').split(':')[1] + ": '" + attrs[j].value + "'}");
        } else {
          let n = tmp.data.lastIndexOf("}");
          tmp.data = tmp.data.slice(0, n) + tmp.data.slice(n).replace("}",
            "," + attrs[j].name.replace(/-/g, '_').split(':')[1] + ": '" + attrs[j].value + "'}");
  ...
  if (!div) {
    ...
    let code = '';
    if (has_id !== -1) {
      code = 'let ' + id + ' = ' + tmp.id + '[' + i + ']' + ' = new Vue({' + 'el: \'#' + id + '\'';
    } else {
      code = tmp.id + '[' + i + '] = new Vue({' + 'el: \'#' + id + '\'';
    }
    ...
    $(script).attr('sue_id', tmp.id + '-' + i);
    $("body").append(script);
    realSauce.size += 1;

  } else {
    console.error(`You cannot render this item because id: ${id} has been exist on the page now`);
  }

  $(parent).attr('status', 'rendered');

  console.log(`Finish rendering <${tmp.name}> id: ${tmp.id}[${i}]`);
}
