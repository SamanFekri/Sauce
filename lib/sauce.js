...
function render(temp) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    let needRender = true;
    console.log($(items[i]).attr('status'));
      ...
      for (let j = 0; j < attrs.length; j++) {
        if (attrs[j].name !== 'id') {
          if (tmp.data == '') {
            tmp.data = 'data: {}';
            let n = tmp.data.lastIndexOf("}");
            tmp.data = tmp.data.slice(0, n) + tmp.data.slice(n).replace("}", attrs[j].name.replace(/-/g, '_') + ": '" + attrs[j].value + "'}");
          } else {
            let n = tmp.data.lastIndexOf("}");
            tmp.data = tmp.data.slice(0, n) + tmp.data.slice(n).replace("}", "," + attrs[j].name.replace(/-/g, '_') + ": '" + attrs[j].value + "'}");
          }
